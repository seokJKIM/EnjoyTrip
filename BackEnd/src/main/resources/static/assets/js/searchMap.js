...
fetch(root + "/place/sido",config)
.then((response) => response.json())
.then((data) => makeOption(data));
  
  function enterKey() {
	  if(window.event.keyCode == 13){
    	  let searchUrl = root + "/place/searchPlace";
  ...
  function makeOption(data) {
    ...
    async function makeGugun(code){
  	  let fetched = await fetch(root + "/place/gugun?code="+code, config);
    ...
    document.getElementById("btn-search").addEventListener("click", () => {
      ...
      async function makeList(searchUrl) {
        let fetched = await fetch(searchUrl, config);
